...
#[derive(Parser, Debug)]
struct Args {
    #[clap(short, long, value_parser, default_value = "/dev/tty.usbserialCF00000015")]
    arg_device: String,
}
                            ...
                            0x80 => {
                                ...
                                if some_buf.len() > 2 {
                                    match some_buf[2] {
                                        ...
                                        0x04 => println!("right key pressed"),
                                        0x03 => println!("left key pressed"),
                                        0x01 => {
                                            println!("up key pressed");
                                            some_packet.set_command(0x0b);
                                            some_packet.add_data(&[0x01, 0x01]);
                                        }
                                        0x02 => println!("down key pressed"),
                                        _ => println!("key released"),
                                    }
                                }
                                //only do this if we actually are sending something
                                if !some_packet.empty() {
